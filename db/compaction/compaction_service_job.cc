...
CompactionJob::ProcessKeyValueCompactionWithCompactionService(
  ...
  // (external_path) or carry a read-time GSN override run LOCALLY: the remote
  // compaction request does not carry these FileDescriptor extensions, so a
  // remote CSA would read the wrong path or use the files' original sequence
  // numbers and could merge stale src versions over newer dst writes.
